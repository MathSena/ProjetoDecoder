...
public class CourseUserController {
  ...
  @Autowired AuthUserClient authUserClient;

  @Autowired CourseService courseService;

  @Autowired CourseUserService courseUserService;
  ...
  public ResponseEntity<Object> saveSubscriptionUserInCourse(
    ...
    if (courseUserService.existsByCourseAndUserId(
        courseModelOptional.get(), subscriptionDto.getUserId())) {
      return ResponseEntity.status(HttpStatus.CONFLICT).body("Error: subscription already exists!");
    }
    try {
      responseUser = authUserClient.getOneUserById(subscriptionDto.getUserId());
      if (Objects.requireNonNull(responseUser.getBody())
              .getUserStatus().equals(UserStatus.BLOCKED)) {
